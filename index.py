...
app.secret_key = '101-783-093'
app.permanent_session_lifetime = False

mysql = MySQL()
app.config['MYSQL_DATABASE_HOST'] = 'mysql://admin:Medico2581895@medico.cv0gq8eacxgc.us-east-2.rds.amazonaws.com:3306/medico'
app.config['MYSQL_DATABASE_USER'] = 'admin'
# Puede omitirse esta parte si no tiene contraseña
app.config['MYSQL_DATABASE_PASSWORD'] = 'Medico2581895'
app.config['MYSQL_DATABASE_DB'] = 'medico'
# Configurar el conjunto de caracteres
app.config['MYSQL_DATABASE_CHARSET'] = 'utf8mb4'
# Configurar la clase de cursor
app.config['MYSQL_DATABASE_CURSORCLASS'] = 'mysql.connector.cursor.MySQLCursor'
mysql.init_app(app)
...
# se define la funcion indicando que si el usuario no se encuentra autenticado este
...
def index():
    ...
    if request.method == 'POST':
        ...
        datos = (_documento, _tipo, _nombre, _edad, _genero, _correo_electronico, _direccion, _telefono,
                 _eps, _cargo, _acompanante, _parentesco, _telefono_acompanante, _motivo_consulta, _ultima_revision)
        ...
        cursor.execute(
            "SELECT documento FROM pacientes WHERE documento = %s", (_documento,))
...
def consulta():
    ...
    sql = """
        ...
        LEFT JOIN antecedentes a ON a.paciente_documento = p.documento
        LEFT JOIN mov_consulta mc ON mc.paciente_documento = p.documento
        LEFT JOIN vista v ON v.paciente_documento = p.documento;
    """
    conn = mysql.connection
    cursor = conn.cursor()
...
def editar(id):
    ...
    return render_template('editar.html', paciente=paciente, user_authenticated=user_authenticated)
...
def eliminar(id):
    ...
    cursor.execute(
        "DELETE FROM antecedentes WHERE paciente_documento=%s", (id))
    conn.commit()

    # Eliminar de la tabla 'mov_consulta'
    cursor.execute(
        "DELETE FROM mov_consulta WHERE paciente_documento=%s", (id))
    conn.commit()
